...
function doAnimations(this: ViewHook, delayTime: number, maxItems: number, elToRemove?: HTMLElement) {
  const ts = [];
  let toasts = Array.from(document.querySelectorAll<HTMLElement>('#toast-group [phx-hook="LiveToast"]'))
    .map((t) => {
      if (isHidden(t)) {
        return null;
      } else {
